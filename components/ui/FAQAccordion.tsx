    ...
    <div className="flex flex-col gap-2">
      {filtered.map((item, i) => (
        <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          ...
          >
            <span className="min-w-0 pr-4 font-medium leading-snug text-gray-900 text-balance">{item.question}</span>
          ...
          <AnimatePresence>
            {open === i && (
              ...
              >
                <div className="px-5 pb-4 text-gray-600 text-sm leading-relaxed border-t border-gray-50 text-pretty">
                  {item.answer}
                </div>
              </motion.div>
            )}
