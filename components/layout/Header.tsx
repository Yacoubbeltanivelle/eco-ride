"use client";
import Link from "next/link";
import Image from "next/image";
import { useState } from "react";
import { Menu, X, Phone } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { asset } from "@/lib/utils";
import { useSettings } from "@/lib/settings-context";

const ALL_NAV_LINKS = [
  { href: "/catalogue",      label: "Catalogue",  feature: null },
  { href: "/location-vtc",   label: "Louer",      feature: "feature_vtc" },
  { href: "/achat-occasion", label: "Acheter",    feature: "feature_occasion" },
  { href: "/mandataire-auto",label: "Commander",  feature: "feature_mandataire" },
  { href: "/simulateur",     label: "Simulateur", feature: "feature_simulator" },
  { href: "/blog",           label: "Blog",       feature: "feature_blog" },
] as const;

export default function Header() {
  const [open, setOpen] = useState(false);
  const settings = useSettings();

  const navLinks = ALL_NAV_LINKS.filter(l =>
    l.feature === null || settings[l.feature as keyof typeof settings] !== false
  );

  const phone        = settings.phone?.replace(/\s/g, "") ?? "+33667489562";
  const phoneDisplay = settings.phone_display ?? "06 67 48 95 62";

  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 h-14 sm:h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center" aria-label="ECO RIDE - Accueil">
          <Image
            src={asset("/brand/ecoride-wordmark-blue.svg")}
            alt="ECO RIDE"
            width={180}
            height={50}
            priority
            className="h-10 w-auto object-contain sm:h-12"
          />
        </Link>

        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-700">
          {navLinks.map(l => (
            <Link key={l.href} href={l.href} className="hover:text-sky-500 transition-colors">{l.label}</Link>
          ))}
        </nav>

        <div className="hidden md:flex items-center gap-3">
          <a href={`tel:${phone}`} className="flex items-center gap-1.5 text-sm font-medium text-gray-700 hover:text-sky-500">
            <Phone className="w-4 h-4" />
            {phoneDisplay}
          </a>
          <Link href="/demande" className="px-4 py-2 rounded-full text-sm font-semibold text-white" style={{ background: "var(--eco-green)" }}>
            Demande rapide
          </Link>
        </div>

        <button type="button" className="md:hidden p-2 rounded-lg" onClick={() => setOpen(!open)}>
          {open ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>
      </div>

      <AnimatePresence>
        {open && (
          <motion.div
            initial={{ opacity: 0, y: -8 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-3"
          >
            {navLinks.map(l => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
                className="py-2 text-base font-medium text-gray-800 border-b border-gray-50">
                {l.label}
              </Link>
            ))}
            <Link href="/demande" onClick={() => setOpen(false)}
              className="mt-2 text-center py-3 rounded-full font-semibold text-white"
              style={{ background: "var(--eco-green)" }}>
              Demande rapide
            </Link>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
  );
}
